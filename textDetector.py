...
class BoundingBox:
    ...
    def detect_characters(self, image, putText=False):
        ...
        for box_info in boxes.splitlines():
            ...
            # print(box_info)
            x, y, w, h = int(box_info[1]), int(box_info[2]), int(box_info[3]), int(
                box_info[4])  # Get the coordinates to draw boxes
            cv2.rectangle(image, (x, image_height - y), (w, image_height - h), (0, 0, 255), 2)  # Draws the boxes
            if putText == True:
                cv2.putText(image, box_info[0], (x, image_height - y + 25), cv2.FONT_HERSHEY_COMPLEX, 1, (50, 50, 255),
                            2)
    ...
    def detect_words(self, image, putText=False):
        ...
        for count, box_info in enumerate(boxes.splitlines()):
            if count != 0:
                ...
                # print(box_info)
                if len(box_info) == 12:
                    x, y, w, h = int(box_info[6]), int(box_info[7]), int(box_info[8]), int(
                        box_info[9])  # Get the coordinates to draw boxes
